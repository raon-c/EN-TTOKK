    ...
    match event_type {
        "system" => {
            let session_id = event
                ...
                .map(ToOwned::to_owned);
            vec![make_chunk(
                "start", None, None, None, None, session_id, None,
            )]
        }
...
pub fn chat_check_status() -> Result<ChatStatusResponse, String> {
...
pub fn chat_start_stream(app: AppHandle, input: ChatStreamStartInput) -> Result<(), String> {
...
pub fn chat_cancel_stream(request_id: String) -> Result<bool, String> {
    cancelled_chat_streams()
        .lock()
