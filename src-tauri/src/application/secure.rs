...
pub fn get_jira_token() -> Result<Option<String>, String> {
    let entry =
        Entry::new(JIRA_TOKEN_SERVICE, JIRA_TOKEN_USERNAME).map_err(|error| error.to_string())?;
...
pub fn remove_jira_token() -> Result<(), String> {
    let entry =
        Entry::new(JIRA_TOKEN_SERVICE, JIRA_TOKEN_USERNAME).map_err(|error| error.to_string())?;
