...
fn extract_content(message: &MessageContent) -> String {
    match message {
        ...
        MessageContent::Complex(c) => match &c.content {
            ...
            Some(MessageBody::Array(blocks)) => blocks
                .iter()
                .filter_map(|block| {
...
pub fn list_claude_projects() -> Result<Vec<String>, String> {
        ...
        for entry in entries.flatten() {
            ...
            if let Ok(jsonl_entries) = fs::read_dir(&project_dir) {
                for jsonl_entry in jsonl_entries.flatten() {
                    ...
                    if path.extension().map(|e| e == "jsonl").unwrap_or(false) {
                        if let Ok(items) = read_jsonl_activities(&path, &date, &subscribed_folders)
                        {
...
pub fn get_claude_activity_dates(
    subscribed_folders: Vec<String>,
    year: i32,
    month: u32,
