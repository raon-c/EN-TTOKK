...
pub async fn jira_test_connection(
    ...
    .await
    .map_err(|error| format!("Jira connection test task failed: {}", error))?
}

#[tauri::command]
#[specta::specta]
pub async fn jira_list_issues(params: JiraRequestInput) -> Result<JiraIssuesListResponse, String> {
    validate_email(&params.email)?;
    validate_api_token(&params.api_token)?;
    let normalized_base_url = normalize_base_url(&params.base_url)?;
    ...
    .await
    .map_err(|error| format!("Jira issues fetch task failed: {}", error))?
}
