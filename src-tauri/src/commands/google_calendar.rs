...
fn prune_oauth_results() -> Result<(), String> {
    ...
    results.retain(|_, result| current.saturating_sub(result.received_at_ms) <= OAUTH_RESULT_TTL_MS);
    Ok(())
}
...
#[tauri::command]
#[specta::specta]
pub async fn google_prepare_oauth(_state: String) -> Result<(), String> {
...
#[tauri::command]
#[specta::specta]
pub async fn google_poll_oauth_result(state: String) -> Result<GoogleAuthResult, String> {
...
pub async fn google_exchange_token(
    ...
    .await
    .map_err(|error| format!("Google token exchange task failed: {}", error))?
}

#[tauri::command]
#[specta::specta]
pub async fn google_list_events(params: GoogleEventsInput) -> Result<HttpProxyResponse, String> {
    if params.access_token.trim().is_empty() {
        return Err("Access token is required".to_string());
    }

    if params.sync_token.is_none()
        && (params.time_min.as_deref().unwrap_or_default().trim().is_empty()
            || params.time_max.as_deref().unwrap_or_default().trim().is_empty())
    {
    ...
    .await
    .map_err(|error| format!("Google events request task failed: {}", error))?
}
